...
class Dataset:
  ...
  def printDataset(self, colWidth=10, otherInstances=None):
    ...
    for idx in range(len(self.revAttributeLU)):
      attributeName = self.revAttributeLU[idx]
      print(attributeName[0:(colWidth-1)].ljust(colWidth), colNames.append(attributeName))
    if self.classification:
    ...
    for instance in instanceList:
      for name in colNames:
        ...
        print(valStr[0:(colWidth-1)].ljust(colWidth))
      if self.classification:
...
def unitTest():
  ...
  file called 'sample.data' in the ./data directory, though you can specify a different file on the command line.
  """
